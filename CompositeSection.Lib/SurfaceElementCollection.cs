...
{
    /// <summary>
    /// Represents a collection of <see cref="SurfaceElement"/>s.
    /// </summary>
    [Serializable]
    public class SurfaceElementCollection : ElementCollection<SurfaceElement>
    {
        /// <inheritdoc />
        public override ElementCollection<SurfaceElement> DeepClone()
        {
            var buf = new SurfaceElementCollection();

            foreach (var elm in this)
            {
                buf.Add(elm.Clone() as SurfaceElement);
            }
